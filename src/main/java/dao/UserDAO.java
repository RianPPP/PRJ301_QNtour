...
public class UserDAO extends DBContext {
    ...
    public void insertUser(String fullname, String phone, String address, String email, String password, int gender) {
        try {
            ...
            SendMail.send(email, "Verify SignUp InfinityShop", messageRegister(fullname, phone, address, email, randomKey));
            ps.executeUpdate();
        } catch (Exception e) {
        }
    }
    ...
    private String messageRegister(String fullname, String phone, String address, String email, String randomKey) {
        return "<!DOCTYPE html>\n"
                ...
                + "                                    <div style=\"background:rgb(255,248,204);border:1px solid rgb(255,140,0);padding:10px;border-radius:3px 3px 0px 0px;font-size:11px;font-family:'Courier New',Courier,monospace\" align=\"center\"> <a title=\"Đường dẫn kích hoạt tài khoản\" href=\"http://localhost:8080/InfinityShop/verify?action=register&email=" + email + "&key=" + randomKey + "\" style=\"text-decoration:none;color:#252525\" target=\"_blank\">http://localhost:8080/InfinityShop/verify?action=register&email=" + email + "&key=" + randomKey + "</a>\n"
                + "                                    </div>\n"
                + "                                </td>\n"
                + "                            </tr>\n"
                + "                        </tbody>\n"
                ...
                + "\n"
                + "</html>";
//        return "To confirm registration please click here: http://localhost:8080/InfinityShop/verify?email=" + email + "&key=" + codeRandom;
    }
    ...
    public void insertKey(String email) {
        try {
            ...
            ps.setString(2, email);
            SendMail.send(email, "Verify reset password InfinityShop", messageResetPass(email, randomKey));
            ps.executeUpdate();
        } catch (Exception e) {
        }
    }
    ...
    private String messageResetPass(String email, String randomKey) {
        return "<!DOCTYPE html>\n"
                ...
                + "                            <tr>\n"
                + "                                <td style=\"padding:10px 20px 12px 20px\">\n"
                + "                                    <div style=\"background:rgb(255,248,204);border:1px solid rgb(255,140,0);padding:10px;border-radius:3px 3px 0px 0px;font-size:11px;font-family:'Courier New',Courier,monospace\" align=\"center\"> <a title=\"Đường dẫn kích hoạt tài khoản\" href=\"http://localhost:8080/InfinityShop/verify?action=forgot&email=" + email + "&key=" + randomKey + "\" style=\"text-decoration:none;color:#252525\" target=\"_blank\">http://localhost:8080/InfinityShop/verify?action=forgot&email=" + email + "&key=" + randomKey + "</a>\n"
                + "                                    </div>\n"
                + "                                </td>\n"
                + "                            </tr>\n"
                + "                        </tbody>\n"
