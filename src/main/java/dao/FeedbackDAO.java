...
public class FeedbackDAO extends DBContext {
    ...
    public ArrayList<Feedback> getFeedbackBypid(int pid) {
        ...
        try {
            String sql = "select fb.*, u.* from feedback fb, [user] u where product_id= ? and u.user_id = fb.user_id and fb.active = 1";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setInt(1, pid);
    ...
    public String getNumberFeedbackByProId(int pid) {
        try {
            String sql = "  select round(avg( Cast(rated as Float)),1),count(*) from feedback where product_id =? and active = 1";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setInt(1, pid);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
    ...
    public ArrayList<Feedback> getAllFeedbacks() {
        ArrayList<Feedback> FeedbackList = new ArrayList<>();
        try {
            String sql = "select feedback_id, fullname,f.img,  title, rated, f.active, email, phone, f.fb_content from feedback f join product p on f.product_id = p.product_id join [user] u on u.user_id = f.user_id "
    ...
    public ArrayList<Feedback> getFeedbackByConditions(String star, String status, String fullname, String product, int index) {
        ArrayList<Feedback> FeedbackList = new ArrayList<>();
        try {
            String sql = "select feedback_id, fullname,f.img,  title, rated, f.active, email, phone, f.fb_content from feedback f join product p on f.product_id = p.product_id \n"
