...
public class BlogDAO extends DBContext {
    ...
    public ArrayList<Blog> getRecentPost() {
        ...
        try {
            String sql = "select top 5 blog_title,substring(description,0,70),img,blog_id from blog b,setting s where b.active = 1  and s.setting_id = b.blog_category order by post_date desc";
    ...
    public ArrayList<Blog> getHotPost() {
        ArrayList<Blog> bloList = new ArrayList<>();
        try {
            String sql = "select top 6 blog_title,substring(description,0,70),img,blog_id from blog b,setting s where b.active = 1  and s.setting_id = b.blog_category order by views desc";
            PreparedStatement stm = connection.prepareStatement(sql);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                Blog b = new Blog(rs.getString(1), rs.getString(2), rs.getString(3), rs.getInt(4));
